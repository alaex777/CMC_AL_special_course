...
def make_dict(set1: Set, list1: List) -> Dict:
    params = dict()
    tmpp = set()
    for i in list1:
        tmp = set()
...
def train(
    ...
    pos_params = make_dict(pos_set, positive)
    neg_params = make_dict(neg_set, negative)

    res = [pos_set, neg_set, pos_params, neg_params, pos_length, neg_length]
...
def classify(texts: List[str], params: Any) -> List[str]:
    ...
    for i in inp:
        pos_p = sum(math.log(pos_params[j]) for j in i if j in pos_set) * pos_length / (pos_length + neg_length)
        neg_p = sum(math.log(neg_params[j]) for j in i if j in neg_set) * neg_length / (pos_length + neg_length)
        if pos_p > neg_p:
            res.append("pos")
        else:
            res.append("neg")
